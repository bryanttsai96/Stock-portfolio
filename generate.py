...
function sBar(s,w){{const c=sc(s);w=w||80;return`<div class="sbw"><div class="sb" style="width:${{w}}px"><div class="sf fill-${{c}}" style="width:${{s}}%"></div></div><span class="sn score-${{c}}">${{s}}</span></div>`;}}
...
function renderTbody(id,list){{
  ...
  tb.innerHTML=list.map(s=>`<tr>
    ...
    <td>${{sBar(s.ai)}}</td><td>${{sBar(s.fin,65)}}</td><td>${{sBar(s.growth_s,65)}}</td>
    <td>${{sBar(s.valuation,65)}}</td><td>${{sBar(s.financial,65)}}</td><td>${{sBar(s.market,65)}}</td>
...
function openDetail(ticker){{
  ...
  const dimRows=dims.map(([label,key,max,note])=>{{
    const val=s[key]||0,c=sc(val),pct=Math.round(val/max*100);
